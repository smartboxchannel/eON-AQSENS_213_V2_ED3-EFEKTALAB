...
boolean Adafruit_SGP40::heaterOff(void) {
  uint8_t command[2];
  command[0] = 0x36;
  command[1] = 0x15;
  return readWordFromCommand(command, 2, 1);
}
...
void Adafruit_SGP40::get_States(void) {
  
  VocAlgorithm_get_states(&voc_algorithm_params, &stat1, &stat2);
  
  stat1bytes[0] = (stat1 & 0x000000FF);
  stat1bytes[1] = (stat1 & 0x0000FF00) >> 8;
  stat1bytes[2] = (stat1 & 0x00FF0000) >> 16;
  stat1bytes[3] = (stat1 & 0xFF000000) >> 24;
  
  stat2bytes[0] = (stat2 & 0x000000FF);
  stat2bytes[1] = (stat2 & 0x0000FF00) >> 8;
  stat2bytes[2] = (stat2 & 0x00FF0000) >> 16;
  stat2bytes[3] = (stat2 & 0xFF000000) >> 24;
}

void Adafruit_SGP40::set_States(int32_t st1, int32_t st2) {
  
  VocAlgorithm_set_states(&voc_algorithm_params, st1, st2);
